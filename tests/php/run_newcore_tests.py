#!/usr/bin/env python3
"""PHP NewCore benchmark test runner.

Tests that PHP NewFunction (code=4/5) correctly triggers NewCore second-pass
scanning for cross-file wrapped sink functions.

Usage:
    cd /path/to/Kunlun-M
    source /path/to/venv/bin/activate
    python tests/php/run_newcore_tests.py
"""
import json
import os
import subprocess
...
# PHP NewFunction: cross-file wrapper functions
test_cases = [
    # newfunction_utils.php: evaluateExpression($expr) { eval($expr); }
    # newfunction_main.php: evaluateExpression($_GET['expr'])
    ('newfunction_main.php', True,
     'CVI-1009 eval: evaluateExpression($_GET[expr]) via newfunction_utils',
     ['CVI-1009']),
]
