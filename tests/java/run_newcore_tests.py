#!/usr/bin/env python3
"""Java NewCore benchmark test runner.

Tests that Java NewFunction (code=5) correctly triggers NewCore second-pass
scanning for cross-file wrapped sink functions.

Note: Java NewCore only applies to rules with match_mode mm_regex_only_match
or mm_regex_return_regex. Rules using java-function-param-regex or
java-function-param-controllable are handled by the AST engine directly.

Usage:
    cd /path/to/Kunlun-M
    source /path/to/venv/bin/activate
    python tests/java/run_newcore_tests.py
"""
import json
import os
import subprocess
...
# Java NewCore: cross-file wrapper functions (static utility classes)
# NewCore works for mm_regex_only_match rules (6003, 6004, 6032, 6033, etc.)
test_cases = [
    # ExecUtils.executeCommand(cmd) -> Runtime.exec()
    # CVI-6003: mm_regex_only_match, sink=Runtime.exec
    ('MainServlet.java', True,
     'CVI-6003 命令注入: ExecUtils.executeCommand(cmd) via getParameter',
     ['CVI-6003']),
    # FileUtils.readConfig(filePath) -> FileReader/new File()
    # CVI-6004: mm_regex_only_match, sink=FileReader/FileInputStream
    ('PathServlet.java', True,
     'CVI-6004 路径穿越: FileUtils.readConfig(file) via getParameter',
     ['CVI-6004']),
]


def run_scan():
    """Run ci_scan on the entire Java test directory and return JSON results."""
