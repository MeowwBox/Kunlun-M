#!/usr/bin/env python3
"""Python NewCore benchmark test runner.

Tests that Python NewFunction (code=4/5) correctly triggers NewCore second-pass
scanning for cross-file wrapped sink functions.

Usage:
    cd /path/to/Kunlun-M
    source /path/to/venv/bin/activate
    python tests/python/run_newcore_tests.py
"""
import json
import os
import subprocess
...
# Python NewFunction: cross-file wrapper functions
test_cases = [
    # 13a: process_command(cmd) { os.system(cmd) }
    # 13b: process_command(sys.argv[2])
    ('13b_cross_file_eval_main.py', True,
     'CVI-7000 os.system: process_command(sys.argv) via cross-file',
     ['CVI-7000']),
]
