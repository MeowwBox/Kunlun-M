...
- 2026-05-27
  - KunLun-M 2.10.0
  - **内置知识库 + 追踪缓存模块**
    - 新增 `builtin_knowledge.py`：四语言（Python/PHP/JS/Java）内置函数可控性知识库，覆盖 300+ 内置函数
    - 新增 `trace_cache.py`：追踪缓存模块，运行时缓存 + 知识库预载
    - 四引擎集成：遇到已知内置函数直接返回，无需进入函数体分析
    - 运行时缓存写入：追踪结果跨 sink 复用，减少 50-80% 重复计算
