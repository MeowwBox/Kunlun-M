...
- 2026-05-27
  - KunLun-M 2.10.3
  - **Go 引擎增强：跨函数跨文件污点追踪 + 多项 Bug 修复**
    - **deps 机制**：仿照 PHP/Python 引擎实现函数体封闭作用域分析
      - `function_back_go()`: 查找函数定义，形参→实参映射，分析 return 语句
      - `_analyze_return_deps_go()`: 返回 `('deps', [caller_var_names])` 让上层继续追踪
      - `_find_function_def_in_lines()`: 正则匹配 `func Name(` 和 `func (receiver) Name(` 定义
      - 跨文件函数查找：通过 `_ast_object_singleton.pre_result` 搜索其他 .go 文件
      - `_scan_function_stack` 递归保护，防止 A→B→A 循环
    - **复合表达式分解**：`_extract_var_names_from_expr()` 支持字符串拼接、fmt.Sprintf、函数透传
    - **嵌套括号参数提取**：`_extract_args_with_nesting()` 替代 `[^)]*` 正则，正确处理 `db.Query("SELECT..." + userId)`
    - **grep 行号修复**：`utils/file.py` 的 `grep()` 和 `multi_grep_content()` 改用 `content[:m.start()].count('\n')` 替代 `split(data)[0]`，修复多个相同匹配行号错误
    - **matcher.py 修复**：Go 扫描优先使用 `vul_function` 列表，避免 `rstrip('(')` 破坏正则模式
    - Benchmark 检出率：1 → 41 个漏洞（跨函数跨文件全部打通）
    - CI 回归测试通过（PHP/Python/Go 均无退化）
- 2026-05-27
  - KunLun-M 2.10.2
  - **新增 Go 语言扫描支持**
